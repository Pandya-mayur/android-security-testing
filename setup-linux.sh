#!/bin/bash
#
# Android Security Testing Setup Script for Linux
#
...
Android Security Testing Setup Script
...
install_system_ca() {
    ...
    if ! openssl x509 -inform DER -in "$cert_path" -out "$pem_path" 2>/dev/null; then
        cp "$cert_path" "$pem_path"
    fi

    local hash=$(openssl x509 -inform PEM -subject_hash_old -in "$pem_path" -noout 2>/dev/null || \
    ...
    openssl x509 -inform PEM -in "$pem_path" -out "$system_cert_path" -outform PEM
...
get_host_ip() {
    # Try various methods to get host IP
    local ip=""
...
if ! $SKIP_CERT; then
    ...
    if ! check_command openssl; then
        log ERROR "OpenSSL not found. Install with: sudo apt install openssl"
        exit 1
    fi
    log OK "OpenSSL found"

    install_system_ca "$cert_path"
...
log INFO "  ./setup-linux.sh --cleanup"
echo ""
